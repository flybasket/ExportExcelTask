package com.wisewater.framework;

import org.springframework.stereotype.Service;

import com.wisewater.framework.vo.ITaskProcesser;
import com.wisewater.framework.vo.JobInfo;
...
public class PendingJobPool {
	...
	private static class PendingQueryDataTask<T, R> implements Runnable {

		private JobInfo<R> jobInfo;
		private T processData;
		private final PendingJobPool parent;

		public PendingQueryDataTask(JobInfo<R> jobInfo, T processData, PendingJobPool parent) {
			super();
		...
		public void run() {
			R r = null;
			// 取得任务的处理器
			ITaskProcesser<T, R> taskProcesser = (ITaskProcesser<T, R>) jobInfo.getQueryDataTaskProcesser();
			...
			try {
				...
				boolean state = jobInfo.addQueryDataTaskResultAndReturnCompleteState(result);
				if (state) {
					parent.noticeTask(jobInfo.getJobName(), jobInfo.getQueryDataList());
					jobInfo.clearQueryData();
				}
			}
		}
	}
	...
	// 调用者提交工作中的任务
	public <T, R> void putQueryDataTask(String jobName, T t) {
		JobInfo<R> jobInfo = getJob(jobName);
		PendingQueryDataTask<T, R> task = new PendingQueryDataTask<T, R>(jobInfo, t, this);
		taskExecutor.execute(task);
	}

	public <T, R> void noticeTask(String jobName, R r) {

		System.out.println("数据获取完成... ");

		JobInfo<R> jobInfo = getJob(jobName);
		List<String> list = Arrays.asList(jobName, jobInfo.getExtendJobInfo());
		QueryDataItemVo<List<String>, R> queryDataItemVo = new QueryDataItemVo<List<String>, R>();
		queryDataItemVo.setType(list);
		queryDataItemVo.setData(r);

		PendingTask<T, R> task = new PendingTask<T, R>(jobInfo, (T) queryDataItemVo);
		taskExecutor.execute(task);
	}
	...
	// 调用者注册工作，如工作名，任务的处理器等等
	public <R> void registerJob(String jobName, String extendJobInfo, int jobLength, int queryDataJobLength,
			ITaskProcesser<?, ?> taskProcesser, ITaskProcesser<?, ?> queryDataTaskProcesser, long expireTime) {
		JobInfo<R> jobInfo = new JobInfo<R>(jobName, extendJobInfo, jobLength, queryDataJobLength, taskProcesser,
				queryDataTaskProcesser, expireTime);
		if (jobInfoMap.putIfAbsent(jobName, jobInfo) != null) {
			throw new RuntimeException(jobName + "已经注册！");
		}
	}
	...
	// 判断是否完成
	public <R> boolean isCompleteTask(String jobName) {
		JobInfo<R> jobInfo = getJob(jobName);
		return jobInfo.isComplete();
	}
